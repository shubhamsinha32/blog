...
$(function() {
	...
	/*
	 * SPARKLINE DOCUMENTATION EXAMPLES
 ...
 * to the mousemove event to see * (roughly) how far the mouse has moved * and
 * then updates the display a couple of times a second via * setTimeout()
 */
function drawMouseSpeedDemo() {
	...
	$('html').mousemove(
			function(e) {
				...
				var mousey = e.pageY;
				if (lastmousex > -1) {
					mousetravel += Math.max(Math.abs(mousex - lastmousex), Math
							.abs(mousey - lastmousey));
				}
				lastmousex = mousex;
				lastmousey = mousey;
			});
	var mdraw = function() {
		...
		if (lastmousetime && lastmousetime != timenow) {
			var pps = Math
					.round(mousetravel / (timenow - lastmousetime) * 1000);
			mpoints.push(pps);
